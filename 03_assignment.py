...
# MongoDB class declaration below here

def merge2Dictionaries(a, b):
    temp = a.copy()
    temp.update(b)
    return temp
...
class MangoDB:
    def __init__(self):
        self.__collections = {'default': {'version': 1.0,
                                           'db': 'mangodb', 'uuid': str(uuid.uuid4())}}

    def display_all_collections(self):
        for key in self.__collections:
            print('collection: ', key)
            for mkey in self.__collections[key]:
                print(mkey, self.__collections[key][mkey])

    def add_collection(self, collection_name):
        self.__collections[collection_name] = {}

    def update_collection(self, collection_name, updates):
        self.__collections[collection_name] = merge2Dictionaries(
            self.__collections[collection_name], updates)
    ...
    def list_collections(self):
        print(list(self.__collections.keys()))

    def get_collection_size(self, collection_name):
        return len(self.__collections[collection_name])
    ...
    def get_uuid(self):
        try:
            return self.__collections['default']['uuid']
        # if no default collection, then it must have been deleted
        except KeyError:
            print("The default collection was deleted.")
...
def exercise01():
    '''
        ...
        Make sure to use Python 3 semantics. Make sure the length and width attributes are private and accessible only via getters. Immutable here means 
        that any change to its internal state results in a new Box being returned.
        ...
        A state change means that a new Box is created but the original Box remains untouched.
...
def exercise02():
    ...
    print(db.get_uuid())  # display the uuid of the database
    db.wipe()
    print(db.get_uuid())  # display the uuid of the database
